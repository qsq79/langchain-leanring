...
# -*- coding: utf-8 -*-
"""
LangChain Models 组件高级示例 (LangChain 1.x 版本)
演示自定义模型、异步调用、缓存、错误处理等高级功能

在 LangChain 1.x 中：
- 部分缓存和回调功能已移至 langchain_core
- 支持更好的异步处理
- 推荐使用新的错误处理机制
"""
...
class RobustLLM:
    """健壮的LLM包装器，包含错误处理和重试机制 (LangChain 1.x 版本)"""
    ...
    async def batch_invoke(self, prompts: List[str], **kwargs) -> List[str]:
        """批量异步调用"""
        tasks = [self.ainvoke(prompt, **kwargs) for prompt in prompts]
        results = []
        ...
        for i, task in enumerate(asyncio.as_completed(tasks)):
            try:
                ...
                print(f"批量调用失败: {prompts[i][:30]}... 错误: {e}")
...
async def custom_llm_example():
    ...
    try:
        ...
        # 调用自定义LLM
        prompt = "请解释什么是量子计算？"
        response = await custom_llm._acall(prompt)
        ...
        # 创建异步任务
        tasks = [custom_llm._acall(prompt) for prompt in prompts]
...
async def error_handling_example():
    ...
    try:
        # 尝试异步调用（会失败并重试）
        response = await robust_llm.ainvoke("这是一个测试")
        print(f"响应: {response}")
    except Exception as e:
        print(f"最终失败: {e}")
    print()
...
async def token_cost_tracking_example():
    """Token和成本跟踪示例"""
    print("=== Token和成本跟踪示例 ===")
    ...
    try:
        ...
        print("成本跟踪功能不可用，跳过示例")
    except Exception as e:
        print(f"成本跟踪示例失败: {e}")

async def semantic_similarity_advanced_example():
    """高级语义相似度分析示例"""
    print("=== 高级语义相似度分析 ===")
    ...
    try:
        ...
        for category, docs in documents.items():
            ...
            all_embeddings[category] = await asyncio.gather(*tasks)

        # 为每个查询生成嵌入
        print("生成查询的嵌入...")
        query_tasks = [embeddings.aembed_query(query) for query in queries]
        query_embeddings = await asyncio.gather(*query_tasks)
...
async def model_comparison_example():
    ...
    models = {
        ...
        "chat-gpt-3.5-turbo": ChatOpenAI(model="gpt-3.5-turbo", temperature=0.3),
    }
    ...
    for model_name, model in models.items():
        try:
            ...
            print(f"  响应: {response_text.strip()}")
...
async def streaming_advanced_example():
    """高级流式输出示例"""
    print("=== 高级流式输出示例 ===")
    ...
    try:
        ...
        message = HumanMessage(content="请详细解释量子计算的基本原理和应用前景。")
        print(f"输入: {message.content}")
        print("流式输出: ")
        ...
        print(f"高级流式输出示例失败: {e}")

async def main():
    """主函数，运行所有高级示例"""
    print("LangChain Models 组件高级示例 (LangChain 1.x 版本)")
    print("=" * 60)
    print("注意：此版本包含异步处理和最新的错误处理机制")
    print("=" * 60)
    print()
    ...
    try:
        ...
        # 分析示例
        await token_cost_tracking_example()
        await semantic_similarity_advanced_example()
        await model_comparison_example()
        await streaming_advanced_example()
